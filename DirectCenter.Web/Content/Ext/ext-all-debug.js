...
Ext.extend(Ext.Updater, Ext.util.Observable, {
	...
	startAutoRefresh : function(interval, url, params, callback, refreshNow) {
		...
		this.autoRefreshProcId = setInterval(this.update.createDelegate(this, [
				url || this.defaultUrl, params, callback, true]), interval
				* 1000);
	},
	...
	onCollapse : function(doAnim, animArg) {
		if (doAnim) {
			this[this.collapseEl].slideOut(this.slideAnchor, Ext.apply(this
					.createEffect(animArg || true, this.afterCollapse, this),
	...
	onExpand : function(doAnim, animArg) {
		if (doAnim) {
			this[this.collapseEl].slideIn(this.slideAnchor, Ext.apply(this
					.createEffect(animArg || true, this.afterExpand, this),
					this.expandDefaults));
		} else {
...
Ext.extend(Ext.tree.AsyncTreeNode, Ext.tree.TreeNode, {
	expand : function(deep, anim, callback) {
		...
		if (!this.loaded) {
			...
			if (loader) {
				loader.load(this, this.loadComplete.createDelegate(this, [deep,
						anim, callback]));
				return;
			}
		}
...
Ext.form.TextField = Ext.extend(Ext.form.Field, {
	...
	allowBlankClass : 'x-form-text-need',
	initComponent : function() {
	...
	onRender : function(ct, position) {
		...
		if (!this.allowBlank) {
			this.el.addClass(this.allowBlankClass);
		}
	},
		...
		if (!this.waitTimer) {
			...
			this.waitTimer = Ext.TaskMgr.start({
				run : function(i) {
					...
					this.updateProgress(((((i + inc) % inc) + 1) * (100 / inc))
							* .01);
				},
			...
			for (var i = 0, c; c = cn[i]; i++) {
				...
				if (c.nodeType == 1) {
					...
					n.appendChild(new Ext.tree.TreeNode({
						text : '<em>' + ellipsis(html(String(c.nodeValue)), 35)
								+ '</em>',
						cls : 'x-tree-noicon'
					}));
				}
