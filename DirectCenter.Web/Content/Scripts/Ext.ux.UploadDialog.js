/**
 * This namespace should be in another file but I dicided to put it here for
 * consistancy.
 */
 ...
 * 
 * Transition / output table format: { 'state_1' : { 'event_1' : [ {
 * p|predicate: function, // Transition predicate, optional, default to true. //
 * If array then conjunction will be applyed to the operands. // Predicate
 * signature is (data, event, this). a|action: function|array, // Transition
 * action, optional, default to Ext.emptyFn. // If array then methods will be
 * called sequentially. // Action signature is (data, event, this). s|state:
 * 'state_x', // New state - transition destination, optional, default to //
 * current state. scope: object // Predicate and action scope, optional, default
 * to // trans_table_scope or window. } ] },
 * 
 * 'state_2' : { ... } ... }
 * 
 * @param mixed
 ...
 * @param object
 *            trans_table Transition / output table.
 * @param trans_table_scope
 *            Transition / output table's methods scope.
 */
	...
	initComponent : function() {
		...
		var tt = {
			...
			'ready' : {
				...
				'stop-upload' : [{
				// We are not uploading, do nothing. Can be posted by user only
				// at this state.
				}],
				'hide' : [{
