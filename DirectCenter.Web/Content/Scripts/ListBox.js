...
Ext.namespace("Ext.Ebc");

/**
 * @class Ext.Ebc.ListBoxView A DnD enabled version of Ext.View.
 ...
 * 
 */
Ext.Ebc.ListBoxView = function(config) {
	...
	Ext.Ebc.ListBoxView.superclass.constructor.call(this, Ext.apply(config, {
		border : false
	}));
};

Ext.extend(Ext.Ebc.ListBoxView, Ext.DataView, {
	/**
	...
	afterRender : function() {
		Ext.Ebc.ListBoxView.superclass.afterRender.call(this);
		if (this.dragGroup) {
...
Ext.Ebc.ListBox = Ext.extend(Ext.form.Field, {
	store : null,
	trackMouseOver : true,
	dataFields : [],
	data : [],
	...
	initComponent : function() {
		Ext.Ebc.ListBox.superclass.initComponent.call(this);
		this.addEvents({
			'dblclick' : true,
			'click' : true,
			'change' : true,
	...
	onRender : function(ct, position) {
		var fs, cls, tpl;
		Ext.Ebc.ListBox.superclass.onRender.call(this, ct, position);
		...
		if (Ext.isIE || Ext.isIE7)
			...
			tpl += ' x-unselectable"';
		tpl += '><span>{' + this.displayField + '}</span></div></tpl>';

		this.view = new Ext.Ebc.ListBoxView({
			multiSelect : true,
			store : this.store,
...
Ext.reg("listbox", Ext.Ebc.ListBox);

Ext.Ebc.MListBox = Ext.extend(Ext.form.Field, {
	msWidth : 200,
	msHeight : 300,
	hideNavIcons : false,
	imagePath : "../Content/Scripts/images",
	iconUp : "up2.gif",
	iconDown : "down2.gif",
	iconLeft : "left2.gif",
	...
	initComponent : function() {
		Ext.Ebc.MListBox.superclass.initComponent.call(this);
		this.addEvents({
			'rowdblclick' : true,
			'change' : true
		});
	...
	onRender : function(ct, position) {
		Ext.Ebc.MListBox.superclass.onRender.call(this, ct, position);

		this.fromMultiselect = new Ext.Ebc.ListBox({
		...
		this.toMultiselect = new Ext.Ebc.ListBox({
...
Ext.reg("mlistbox", Ext.Ebc.MListBox);